...
def create_decision_tree() -> graphviz.Digraph:
...
# Функція для рендерингу та збереження дерева рішень
def render_decision_tree(tree: graphviz.Digraph, filename: str) -> None:
    tree.render(filename)

# Створення дерева рішень
decision_tree = create_decision_tree()

# Рендеринг та збереження
render_decision_tree(decision_tree, 'full_decision_tree')
